  ...
  constructor() {
    sqlite
      ...
      .then((db) => {
        ...
        this.db = db;
        db.exec(
          "create table if not exists treenodes (id integer primary key autoincrement , name text, parent_id int references treenodes(id), type text, text text, parent_type text);"
        );
